...
<body>
    ...
    <div class="ml-5 mt-2 agent">
        ...
        <a href="{{ route('createAgent') }}" class="btn btn-primary">Create Agent</a>
        </br></br>
        <form action="{{ route('searchAgent') }}" method="get" class="mb-3">
            <div class="input-group">
                <input type="text" name="search" class="form-control"
                    placeholder="Search agents by id, name, phone or email address" value="{{ isset($searchTerm) ? $searchTerm : '' }}">
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $agent->properties()->where('propertyAvailability', 1)->count() }}</td>
                    <td>{{ $agent->status }}</td>
                    <td>
                        <a href="{{ route('AgentDetailsAdmin', $agent->agentID) }}" class="btn btn-primary btn-action">View</a>
                        <a href="{{ route('updateAgent', $agent->agentID) }}" class="btn btn-warning text-white btn-action">Update</a>
                        <a href="#" class="btn btn-danger btn-action" onclick="confirmDelete(
                            '{{ route('deleteAgent', $agent->agentID) }}',
                            '{{ $agent->agentID }}',
                            '{{ $agent->agentName }}',
                            '{{ $agent->agentPhone }}',
                            '{{ $agent->agentEmail }}',
                            '{{ $agent->properties()->where('propertyAvailability', 1)->count() }}'
                        )">Delete</a>
                        </td>
